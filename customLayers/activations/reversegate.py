...
@custom_vjp
def r_gate_function(tensor_input, width=1):
...
def r_gate_function_fwd(tensor_input, width):
    y = r_gate_function(tensor_input, width)
    return y, (tensor_input, width)


def r_gate_function_bwd(res, grad_output):
...
r_gate_function.defvjp(r_gate_function_fwd, r_gate_function_bwd)


class ReverseGateActivation:
    ...
    def __call__(self, tensor_input):
        ...
        return r_gate_function(tensor_input, self.width)
