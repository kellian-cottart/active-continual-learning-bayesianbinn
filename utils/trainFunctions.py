...
File description: Training functions for training models
"""

import equinox as eqx
from optimizers import *
from utils.testFunctions import *
from models import BaseBayesBiNNMLP
import equinox as eqx
...
from utils.activeLearningFunctions import *
...
def loss_fn(model, images, labels, samples=None, rng=None, ewc_online_parameters=None, ewc_streaming_parameters=None, ewc_parameters=None, si_parameters=None, init_state=None, opt_state=None):
    ...
    if isinstance(model, BaseBayesBiNNMLP):
        loss_fn_to_use = bayesbinn_loss_fn
...
def bayesbinn_loss_fn(model, images, labels, samples, rng, init_state=None):
    ...
    def closure(model, samples_rng, init_state):
        ...
        # Copy the structure of the dynamic part of the model
        ...
        key_tree = unflatten(struct_copy, rkeys)
        ...
        noise_tree = map(make_noise, dynamic, key_tree)
        # Reparameterize the dynamic part of the model
        reparam_model_dynamic = map(partial(
        ...
        def loss_fn(model, images, labels, init_state):
            ...
            predictions, state = jax.vmap(partial(model, backwards=True), axis_name="batch", in_axes=(
            ...
            output = jax.nn.log_softmax(predictions, axis=-1) * labels * mask
            # Compute the loss
    ...
    grads = map(partial(grad_on_mu, temperature=model.temperature), grads, eqx.filter(
...
def deterministic_loss_fn(model, images, labels, rng, init_state=None):
    ...
    mask = jnp.ones(labels.shape, dtype=bool)
    if hasattr(model, 'active_learning') and model.active_learning is not None:
        samples = model.active_learning.get("samples", 1)
        labels, predictions, mask = apply_active_learning_strategy(
            model, images, labels, samples, rng, init_state, predictions)
    output = jax.nn.log_softmax(
        predictions, axis=-1).mean(axis=1) * labels * mask
...
def compute_fisher(model, dataset):
    if isinstance(dataset, DataLoader):
        ...
        for i, (images, labels) in enumerate(dataset):
            ...
            fisher = map(lambda x, y: x + y**2, fisher, grads)
            return map(lambda x: x / len(dataset.dataset), fisher)
    else:
