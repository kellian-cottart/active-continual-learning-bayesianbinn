...
parser.add_argument(
    "-w", "--weights", help="Weights of the objective function", type=float, nargs=3, default=[0.5, 0.25, 0.25])
...
def compute_objective(accuracy, task, first_task_accuracies,  weights=(0.5, 0.25, 0.25)):
    weights = jnp.array(weights)
    weights = weights / jnp.sum(weights)
    first_task_accuracies = first_task_accuracies[0].item(
    ) if first_task_accuracies is not None else 0.0
    task_accuracy = accuracy[task].item()
    mean_accuracy = accuracy.mean().item() if isinstance(
        accuracy, jax.Array) else (sum(accuracy) / len(accuracy))[0]
    return first_task_accuracies*weights[0] + mean_accuracy*weights[1] + task_accuracy*weights[2]
...
def update_params(params_dict, dict_type_fn):
    for key in params_dict.keys():
        if isinstance(params_dict[key], dict):
            params_dict[key] = update_params(params_dict[key], dict_type_fn)
            if "type" in params_dict[key]:
                params_dict[key] = assign_value(
                    dict_type_fn, params_dict[key], key)
    return params_dict
...
def objective(trial):
    ...
    # for any configuration keys that are dictionaries, we update their values recursively
    for key in configuration.keys():
        if isinstance(configuration[key], dict):
            configuration[key] = update_params(
                configuration[key], dict_type_fn)
    ...
    for task_id, task in enumerate(pbar):
        ...
        for epoch in epoch_pbar:
            ...
            accuracies = accuracies.reshape(
                configuration["n_tasks"], -1).mean(axis=-1) if not isinstance(accuracies, list) else jnp.array([i.mean() for i in accuracies])
            current_step = task * configuration["epochs"] + epoch
            current_obj = compute_objective(
                accuracies, task_id, first_task_accuracies, WEIGHTS)
    ...
    return compute_objective(accuracies, task_id, first_task_accuracies, WEIGHTS)
