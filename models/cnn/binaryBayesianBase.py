...
class BinaryResidualBlock(Module):
    ...
    norm2: LayerNorm
    downsample: BinaryBayesianConv2D
    ...
    def __init__(self, key, in_ch, out_ch, activation_fn, spatial_size, stride=1, use_bias=False):
        ...
        # First conv
        self.conv1 = BinaryBayesianConv2D(key=k1, in_channels=in_ch, out_channels=out_ch,
                                          kernel_size=3, stride=stride, padding="SAME", use_bias=use_bias)
        self.norm1 = LayerNorm(
            shape=(out_ch, spatial_size, spatial_size), use_weight=False, use_bias=False)
    ...
    def __call__(self, x, key, temperature=1.0, backwards=False):
        k1, k2, k3 = split(key, 3)
        out = self.activation_fn(self.norm1(
            self.conv1.sample(x, key=k1, temperature=temperature)))
        out = self.norm2(self.conv2.sample(
            out, key=k2, temperature=temperature))
        identity = x if self.downsample is None else self.downsample.sample(
            x, key=k3, temperature=temperature)
